...
export default function SettingsClient({ initialProfile }: SettingsClientProps) {
    ...
    useEffect(() => {
        if (initialProfile) {
            ...
            setTimezone(initialProfile.timezone || 'Asia/Jakarta');
            setLanguage(initialProfile.language || 'id');
            setWebhookToken(initialProfile.tenant?.webhook_token || '');
            const savedNotifs = initialProfile.tenant?.settings?.notifications;
            if (savedNotifs) {
